...
## Running on the HAC VM - Moreh Solution Script
...
For this task, I tested with the VOC dataset, the COCO you can do the same, just replace the VOC_ROOT into the COCO_ROOT with the exact root to the COCO path.
The data that I used for train was downloaded via following scripts
```bash
# /bin/bash
# > get_voc.sh
...
Hot fix:
Simply remove above lines and add following to the same file
```
_nms = torchvision.ops.nms
```
